...
import random
import string
from mgt.mex_mgt_fr import *
from headers.heads_fr import *
...
def cx_registNo():
    sql='''#查询贷后状态为正常的贷款用户手机号，产品号写死，实收表数据为空（未还过款）
          select d.REGIST_NO from lo_loan_dtl a left join lo_loan_prod_rel b   on a.LOAN_NO=b.LOAN_NO
          left join cu_cust_reg_dtl d on a.CUST_NO=d.CUST_NO where a.AFTER_STAT='10270002' and b.APP_NO="'''+appNo+'''" and b.PROD_NO='25002400' order by a.INST_TIME desc limit 1;'''
    registNo=DataBase(which_db).get_one(sql)
    registNo=str(registNo[0])
    #print(registNo)
    return registNo
...
def cx_registNo_03():
    sql='''#查询手机号，不在人工或自动审批通过，拒绝状态，202，非空,非撤销状态
...
where a.APPR_TYPE='10290002' and a.APPR_STAT not in('10200001','10200002','10200005','10200006','10200009') and a.APPR_NO='202001' and c.REGIST_NO is not null
...
where d.TRAN_CHAN_NAME='Stp_Payout_FeriaRapidaProd' and d.tran_use='10330002'  and b.APP_NO="'''+appNo+'''" and a.AFTER_STAT='10270003' and a.BEFORE_STAT='10260005' and f.USEABLE='10000001'
order by a.INST_TIME desc limit 1;'''
    phone=DataBase(which_db).get_one(sql)
    return phone
def cx_registNo_052():
    sql='''#查询有还款申请记录，正常状态的贷款，手机号和还款入账账号
select DISTINCT c.REGIST_NO,d.IN_ACCT_NO from pay_tran_dtl d left join lo_loan_dtl a  on d.loan_no=a.loan_no
left join lo_loan_prod_rel b on a.LOAN_NO=b.LOAN_NO left join cu_cust_reg_dtl c on a.CUST_NO=c.CUST_NO left join cu_cust_bank_card_dtl f on a.CUST_NO=f.CUST_NO
where d.TRAN_CHAN_NAME='Stp_Payout_FeriaRapidaProd' and d.tran_use='10330002'  and b.APP_NO="'''+appNo+'''" and a.AFTER_STAT='10270002' and a.BEFORE_STAT='10260005' and f.USEABLE='10000001'
order by a.INST_TIME desc limit 1;'''
    phone=DataBase(which_db).get_one(sql)
    return phone
def cx_registNo_06(loanNo):
    sql='''#查询OXXO还款申请记录
select SHD_TRAN_AMT,tran_order_no,in_acct_no,INST_TIME from pay_tran_dtl t where LOAN_NO="'''+loanNo+'''" and  ACT_TRAN_AMT is null and  TRAN_CHAN_NO='FeriaRapidaProd_Conekta_Payment' order by INST_TIME desc limit 1;'''
    phone=DataBase(which_db).get_one(sql)
    #print(phone)
    return phone
def cx_registNo_07():

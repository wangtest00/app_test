...
class Test_TikTok_Oppo(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        adb_connect(devices_info_tiktok['udid'])                      #连接wifi调试
        huanxing_screen(devices_info_tiktok['udid'])                  #唤醒屏幕
        sildes(devices_info_tiktok['udid'],360, 1400, 360, 1300, 50)  #adb向上滑屏
        appium_start('127.0.0.1', port_tiktok)                        #启动appium服务
    def setUp(self):
        '''每条testcase执行前初始化'''
        print('testcase begin')
        self.driver=devices_object_tiktok.init_devices(port_tiktok, devices_info_tiktok)
        #设置隐式等待为 10s,一旦设置了隐式等待，它则会在整个Web Driver对象的实例声明周期中。
        self.driver.implicitly_wait(10)
        swipeup(self.driver,1000)
    def test_shuaping(self):
        for i in range(10):
    ...
    def tearDownClass(cls):
        adb_disconnect(devices_info_tiktok['udid'])
        appium_stop(port_tiktok)
